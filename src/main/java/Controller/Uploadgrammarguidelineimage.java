...
public class Uploadgrammarguidelineimage extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String test = GrammarguidelinemanageDAO.Uploadimagegrammarguideline(conn, request, response, grammarguidelineid);
		if(test.equals("Thành công")) {
			RequestDispatcher rd = request.getRequestDispatcher("Listgrammarguidelinemanage");
			rd.forward(request, response);
		}
		else {
			request.setAttribute("msggrammarguidelineimage", test);
